...
class TestLoan(BaseCommon):
    ...
    def test_onchange(self):
        loan = self.env["account.loan.oca"].create(
            {
                "name": "LOAN",
    ...
    def test_increase_amount_validation(self):
        ...
        wzd = self.env["account.loan.oca.generate.wizard"].create({})
        action = wzd.run()
        ...
        with self.assertRaises(UserError):
            self.env["account.loan.oca.increase.amount"].with_context(
    ...
    def test_pay_amount_validation(self):
        ...
        self.assertEqual(line.move_ids.state, "posted")
        with self.assertRaises(UserError):
            self.env["account.loan.oca.pay.amount"].with_context(
                default_loan_id=loan.id
            ).create(
                {
                    "amount": (amount - amount / periods) / 2,
    ...
    def test_increase_amount_loan(self):
        ...
        action = (
            self.env["account.loan.oca.increase.amount"]
    ...
    def test_fixed_principal_loan_leasing(self):
        ...
        action = (
            self.env["account.loan.oca.generate.wizard"]
            .create(
                {
                    "date": fields.date.today() + relativedelta(days=1),
                    "loan_type": "leasing",
        ...
        with self.assertRaises(UserError):
            self.env["account.loan.oca.pay.amount"].create(
                {
                    "loan_id": loan.id,
                    "amount": (amount - amount / periods) / 2,
                    "fees": 100,
    ...
    def test_fixed_principal_loan_auto_post_leasing(self):
        ...
        self.assertFalse(line.has_invoices)
        self.assertFalse(line.has_moves)
        self.env["account.loan.oca.generate.wizard"].create(
    ...
    def test_cancel_loan(self):
        ...
        self.assertTrue(line.move_ids)
        self.assertEqual(line.move_ids.state, "posted")
        pay = self.env["account.loan.oca.pay.amount"].create(
    ...
    def post(self, loan):
        ...
        post = (
            self.env["account.loan.oca.post"]
            .with_context(default_loan_id=loan.id)
            .create({})
        )
        post.run()
